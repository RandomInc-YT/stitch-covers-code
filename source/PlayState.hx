...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'tord':
				dialogue = CoolUtil.coolTextFile(Paths.txt('tord/Dialogue'));
			case 'ugh':
				dialogue = CoolUtil.coolTextFile(Paths.txt('ugh/Dialogue'));
		}
			...
			case 'toybox':
				{
						defaultCamZoom = 0.80;
						curStage = 'toybox';
						var swirlsBG:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('TOYBOX REALM BG'));
						swirlsBG.antialiasing = true;
						swirlsBG.scrollFactor.set(0.9, 0.9);
						swirlsBG.active = false;
						add(swirlsBG);

						var toyBG:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('TOYBOX ITEMS'));
						toyBG.antialiasing = true;
						toyBG.scrollFactor.set(0.9, 0.9);
						toyBG.active = false;
						add(toyBG);

						var floorBG:FlxSprite = new FlxSprite(-700, 600).loadGraphic(Paths.image('TOYBOX TILE'));
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curBeat == 13 && curSong == 'Tord' && dad.curCharacter == 'stitch')
			{
				dad.playAnim('shook', true);
			}
